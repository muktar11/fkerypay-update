...
const SalesOrderContent = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${API_ENDPOINT}/api/webcustomer/approve/list/`)
...
useEffect(() => {
  if (selectedCustomerId) {
    ...
    fetch(`${API_ENDPOINT}/commerce/ledger-deposit-view/${selectedCustomerId}/`)
      .then((response) => response.json())
  ...
  const handleFormSubmit = async () => {
    if (!selectedCustomerId) {
      Alert.alert('No customer selected');
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_ENDPOINT}/commerce/remote-sales-order/create/${selectedCustomerId}/${id}/`, {
        method: 'POST',
        headers: {
    ...
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <View style={{ flex: 1, justifyContent: 'center', padding: 18 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
            ...
            >
              <Picker.Item label="-- ደንበኛን ይምረጡ --" value="" />
...
<TouchableOpacity
      ...
      >
        <Text>
          {selectedInventory ? selectedInventory : 'መጋዘን ይምረጡ'}
        </Text>
      </TouchableOpacity>
...
export default function SalesOrderScreen() {
  return (
    ...
    >
      ...
      />
      <Drawer.Screen
        name="የሽያጭ ትዕዛዞችን መመለስ"
        component={SalesOrderReturnScreen}
        options={{
          title: 'የሽያጭ ትዕዛዞችን መመለስ',
        }}
      />
      <Drawer.Screen
        name="የሽያጭ ትዕዛዞች"
