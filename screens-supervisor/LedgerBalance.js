...
export default class LedgerBalanceScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
    /*  tableHead: ['No', 'Date', 'Amount','CSI/CSRI', 'Bank Name', 'Branch Name','Narrative', 'Bank Refence Number', 'Balance'], */
    
      tableHead: ['ቁጥር', 'ቀን', 'የገንዘብ መጠን',
         'የሂሳብ መዝገብ', 'ጸድቋል', 'ተመለሰ', 'መመለሻ_ጉዳይ',
       "የባንክ ስም" ,
        'የቅርንጫፍ ስም', 'አስገቢ ስም', 'የባንክ ማመሳከሪያ ቁጥር', 
        ],
      widthArr: [90, 90, 90, 150, 150, 150,  150, 150, 150, 150, 150, 150, 150, 150,],
      data: [],
  ...
  fetchData = async () => {
    try {
      const response = await fetch(`${API_ENDPOINT}/commerce/ledger-deposit-view/`);
  ...
  <Row
    ...
    data={[
      ...
      item.Deposit_Amount,
      item.Balance,
      item.finance_approved ? "Yes" : "No", // Map boolean to Yes/No
      item.finance_returned ? "Yes" : "No", // Map boolean to Yes/No
      item.return_issue || "-", // Show "-" if null
