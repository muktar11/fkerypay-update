...
const uploadImage = async () => {
  ...
  formData.append('CSI_CSRI_Number', CSI_CSRI_Number || ''); // Make sure values exist
  ...
  try {
    let res = await fetch(`${API_ENDPOINT}/commerce/mobile-ledger-deposit/${selectedCustomerId}/`, {
      method: 'POST',
      body: formData,
      headers: {
    ...
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      ...
      <View style={{ flex: 1, justifyContent: 'center', padding: 18 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
            ...
            >
              <Picker.Item label= {t('"-- ደንበኛን ይምረጡ --" value=" " ')}/>
        ...
        <View>
          ...
          >
            <Text style={{ color: selectedStartDate ? "black" : "gray" }}>
             {selectedStartDate ? selectedStartDate : t('select_date')}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
              ...
              <Text style={{ color: "white" }}>Close</Text>
                ...
                <Animated.View 
                    ...
                    <TouchableOpacity onPress={selectFile}  className="w-full bg-sky-400 p-3 rounded-2xl mb-3">
                        <Text className="text-xl font-bold text-white text-center"> {t(' ፎቶ አያይዝ ')}</Text>
                    </TouchableOpacity>
                </Animated.View>
    ...
    >
      ...
      >
        {loading ? (
          ...
          <Text className="text-xl font-bold text-white text-center"> {t(' አስገባ ')}</Text>
        )}
      </TouchableOpacity>
    </Animated.View>
