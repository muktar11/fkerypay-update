...
const ReturnDepositScreen = () => {
  ...
  const retrieveUserData = async () => {
    try {
      const storedWebId = await AsyncStorage.getItem('web_id');
      setWeb_id(storedWebId);
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setData(fetchedData); // Correctly set the state
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    };

    if (web_id) { // Fetch data only if web_id is not empty
      fetchData();
    }
  }, [web_id]);
...
export default ReturnDepositScreen;
